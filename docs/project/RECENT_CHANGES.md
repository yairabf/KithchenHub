...
- `mobile/scripts/store-release-version.js` backs `npm run verify:store-version`; it rejects `1.0.0`, validates repo versions as semantic `MAJOR.MINOR.PATCH`, and can compare against `STORE_CURRENT_VERSION` / `IOS_CURRENT_STORE_VERSION` values that omit patch or include legacy leading-zero App Store segments.
