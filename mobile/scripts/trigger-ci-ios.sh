#!/usr/bin/env bash
# Triggers GitHub Actions mobile release workflow for iOS.
...
info()    { echo -e "${BLUE}[ci-deploy-ios]${NC} $*"; }
success() { echo -e "${GREEN}[ci-deploy-ios]${NC} $*"; }
warn()    { echo -e "${YELLOW}[ci-deploy-ios]${NC} $*"; }
fail()    { echo -e "${RED}[ci-deploy-ios] ERROR:${NC} $*" >&2; exit 1; }
...
info "Triggering GitHub Actions iOS release workflow..."
gh workflow run mobile-release.yml --ref main -f platform=ios -f submit=true
