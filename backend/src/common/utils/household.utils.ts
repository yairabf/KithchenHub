...
export class HouseholdUtils {
     ...
     */
    static validateHouseholdMembership(householdId: string | undefined): asserts householdId is string {
        if (!householdId) {
            throw new BadRequestException('User must belong to a household to import data');
        }
    }
}
