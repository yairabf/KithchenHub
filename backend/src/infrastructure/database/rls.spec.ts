...
describe('Row Level Security (RLS) Integration Tests', () => {
   ...
   */
  async function fetchStorageColumns(
    client: RawClient,
  ): Promise<StorageColumn[]> {
   ...
   */
  async function ensureHouseholdUploadsBucket(
    client: RawClient,
  ): Promise<void> {
   ...
   */
  async function areHouseholdUploadsPoliciesPresent(
    client: RawClient,
  ): Promise<boolean> {
  ...
  function createStorageInsertPayloadBuilder(columns: StorageColumn[]) {
    ...
    return (params: StorageInsertParams): StorageInsertPayload | null => {
      const values = insertColumns.map((column) =>
        buildColumnValue(column, params),
      );
  ...
  function buildStorageInsertQuery(payload: StorageInsertPayload): Prisma.Sql {
    ...
    const valueSql = Prisma.join(
      payload.values.map((value) =>
        isPrismaSql(value) ? value : Prisma.sql`${value}`,
      ),
    );
        ...
        await expect(
          (tx[collection] as any).create({
            data: {
              householdId: contextB.householdId,
              title:
                collection === 'shoppingList' ? undefined : `Stolen ${label}`,
  ...
  describe.each([
    ...
    it(`should soft-delete ${label} and filter from active queries`, async () => {
      ...
      await prisma.$transaction(async (tx) => {
        await tx.$executeRawUnsafe('SET LOCAL ROLE authenticated');
        await tx.$executeRawUnsafe(
          `SET LOCAL "request.jwt.claims" = '{"sub": "${context.userId}"}'`,
        );
  ...
  describe('Storage RLS for household uploads', () => {
    ...
    let buildInsertPayload:
      | ((params: StorageInsertParams) => StorageInsertPayload | null)
      | null = null;
    ...
    function shouldRunStorageTests(): boolean {
      ...
      if (process.env.ALLOW_STORAGE_RLS_SKIP === 'true') {
        ...
        console.warn(
          `Storage RLS tests skipped: ${storageSkipReason ?? 'unknown reason'}`,
        );
        return false;
      }
    ...
    it.each(insertCases)(
      ...
      async ({ target, shouldSucceed }) => {
        ...
        try {
          const insertPromise = insertStorageObjectAsUser(
            contextA.userId,
            objectName,
          );
          if (shouldSucceed) {
