...
Signed-in mobile app support submissions are backend-first via protected `POST /api/v1/support/tickets`. The backend forwards the ticket by Resend from configured `EMAIL_FROM` to `yair.solutions.19@gmail.com` with `reply_to` set to the submitter contact email. The mobile `Email support instead` fallback still opens the same `mailto:` draft for backend/offline/unauthenticated cases.
